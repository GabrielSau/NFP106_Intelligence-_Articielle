...
def heuristique_manhattan(a, b):
    ...
    Returns:
        int: |dr| + |dc|
    """
    # TODO(1): écrire le calcul Manhattan et retourner un int
    # (r1, c1), (r2, c2) = a, b
    # return ...
    # raise NotImplementedError("TODO(1) : heuristique_manhattan")
    r1, c1 = a
    r2, c2 = b
...
def astar_initialiser(depart, arrivee, w):
    ...
    """
    # TODO(2): créer pq + push, puis retourner le dict état
    # pq = []
    # g0 = 0
    ...
    # return {...}
    # raise NotImplementedError("TODO(2) : astar_initialiser")
    priority_queue = []
    g = {depart: 0}
    parent = {depart: None}
    visite = set()
    ...
    f = g[depart] + w * heuristique_manhattan(depart, arrivee)
    heapq.heappush(priority_queue, (f, g[depart], depart))
...
def astar_faire_une_etape(grille, etat, arrivee, couts):
    ...
    """
    # TODO(3): implémenter toute la logique décrite ci-dessus
    # raise NotImplementedError("TODO(3) : astar_faire_une_etape")

    if etat["termine"]:
        return

    private_queue = etat["priority_queue"]
    g = etat["g"]
    parent = etat["parent"]
    visite = etat["visite"]
    ...
    while private_queue:
        f, g_node, node = heapq.heappop(private_queue)

        if g.get(node, None) != g_node or node in visite:
            continue

        visite.add(node)
        frontiere.discard(node)
        etat["courant"] = node
        ...
        for rr, cc, _ in voisins_4(grille, *node):
            voisin = (rr, cc)
            cout_voisin = cout_case(couts, voisin)
            g_voisin = g[node] + cout_voisin
            if voisin not in g or g_voisin < g[voisin]:
                g[voisin] = g_voisin
                parent[voisin] = node
                f_voisin = g_voisin + w * heuristique_manhattan(voisin, arrivee)
                heapq.heappush(private_queue, (f_voisin, g_voisin, voisin))
...
def reconstruire_chemin(parent, depart, arrivee):
    """
    TODO(4) (optionnel mais recommandé) :
    ...
    """
    # TODO(4): implémenter (ou recopier votre version précédente)
    # raise NotImplementedError("TODO(4) : reconstruire_chemin")
    chemin = []
    cur = arrivee
