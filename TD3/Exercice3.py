...
def astar_initialiser(depart, arrivee):
    """
    Initialisation du Variable Neighborhood Search (VNS).
    """
...
def astar_faire_une_etape(grille, etat, arrivee, couts):
    """
    Une étape de Variable Neighborhood Search.
    """
    ...
    file = deque()
    file.append((courant, 0))

    vus_local = {courant}

    while file:

        pos, dist = file.popleft()
        ...
        for rr, cc, _ in voisins_4(grille, r, c):
    ...
    # Frontière pour l'affichage UI
    etat["frontiere"] = set(voisins) - visite

    meilleur_voisin = None
    meilleur_h = h_courant

    for v in voisins:

        h = heuristique_manhattan(v, arrivee)
    ...
    if meilleur_voisin is not None:
        ...
        cout_transition = cout_case(couts, meilleur_voisin)

        nouveau_g = g[courant] + cout_transition

        if meilleur_voisin not in g or nouveau_g < g[meilleur_voisin]:
            g[meilleur_voisin] = nouveau_g
            parent[meilleur_voisin] = courant

        etat["courant"] = meilleur_voisin
        etat["h_courant"] = meilleur_h

        # retour au petit voisinage
        etat["k"] = 1
        ...
        # voisinage maximal atteint
        if etat["k"] > etat["k_max"]:
            etat["termine"] = True
            etat["trouve"] = False
